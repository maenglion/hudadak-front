...
/* ====== ① 게이지 링 애니메이션 유틸 ====== */
// 0~1 사이 perc를 conic-gradient로 채워줌
  ...
  try {
    ...
    for (let i=start;i<end;i++){
      ...
      hourly.push({
        ts: `${t[i]}:00`,
        pm10, pm25,
        grade: caiGrade(pm10,hourly.pm25) ?? 2,
...
/* ====== ③ 게이지 렌더 (styles.css 기준) ====== */
function renderGauge(data){
  const {pm10=0, pm25=0, display_ts, badges, cai_grade} = data;
  ...
  // max 값 임시로 잡음 (실데이터는 나중에 조정)
  const pm10Perc = Math.min(1, pm10 / 150);  // 150µg/m³ 기준
  const pm25Perc = Math.min(1, pm25 / 75);   // 75µg/m³ 기준
...
function renderLinearBars(d){
  ...
  items.forEach(it=>{
    ...
    el.innerHTML = `
      ...
      <div class="bar-wrapper"><div class="bar-fill" style="width:${v==null?0:Math.min(100, (Number(v)/180)*100)}%"></div></div>
...
function bindTabs() {
  ...
  const initial = document.querySelector('.tab-button.active, .tab-item.tab-item--active')?.dataset.tab
    || btns[0]?.dataset.tab;
...
function renderMain(air){
  ...
  if (descEl){
    descEl.textContent = (air.cai_value!=null) ? `지수 ${air.cai_value}` : '오늘의 대기질 총평입니다.';
  }
}
