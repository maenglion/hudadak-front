...
(function(){
  ...
  // 주요 노드
  const slideMenuOverlay = $('.slide-menu-overlay');   // 오버레이
  const logoButton       = $('.logo-button');          // 로고 버튼
  const menuButton       = $('.menu-button');          // 알림/메뉴 버튼
  const searchButton     = $('.search-button');        // 검색 아이콘
  const searchOverlay    = $('.search-overlay');       // 검색 오버레이
  const menuItems        = $$('.menu-item');           // 아코디언 헤더들

  // ---- 탭 전환 로직 ----
  const ACTIVE = 'active';
  const paneId = (t) => `tab-${t}`;

  function activateTab(target, {fromHash=false}={}){
    if(!target) return;
    ...
    // 공기질 탭일 때 커넥터 재계산(있을 때만)
    if (target === 'air-quality' && typeof window.updatePmConnectors === 'function' && window.__lastAngles){
  ...
  document.addEventListener('click', (e)=>{
    ...
    if(tabBtn){
      e.preventDefault();
      const t = tabBtn.dataset.tab; // "forecast", "air-quality" 등
      if(t) activateTab(t);
      return;
    }

    // 2) 로고 → 슬라이드 메뉴 열기
    if(logoButton && (e.target === logoButton || logoButton.contains(e.target))){
      e.stopPropagation();
    ...
    // 3) 알림/메뉴 → 위치 재조회
    if(menuButton && (e.target === menuButton || menuButton.contains(e.target))){
      e.stopPropagation();
    ...
    // 4) 검색 버튼 → 검색 오버레이 열기
    if(searchButton && (e.target === searchButton || searchButton.contains(e.target))){
      if(searchOverlay) searchOverlay.style.display = 'block';
      return;
    }

    // 5) 슬라이드 메뉴 오버레이 바깥 클릭 → 닫기
    if(slideMenuOverlay && e.target === slideMenuOverlay){
      slideMenuOverlay.style.display = 'none';
      return;
    }
    ...
    // 7) 아코디언(메뉴 아이템)
    const hdr = e.target.closest('.menu-item');
    if(hdr && menuItems.includes(hdr)){
      ...
      menuItems.forEach(other=>{
        if(other !== hdr){
          ...
          if (s && s.classList.contains('sub-menu-box')) s.classList.remove('active');
        }
      });

      // 현재 토글
      hdr.classList.toggle('active');
      if(sub && sub.classList.contains('sub-menu-box')){
        sub.classList.toggle('active');
      }
    }
  });

  // ---- 키보드 접근성 (Tab 버튼 Enter/Space) ----
  ...
  window.addEventListener('DOMContentLoaded', ()=>{
    ...
    const first = $('.tab-button')?.dataset.tab;
    const initial = (hash && document.getElementById(paneId(hash))) ? hash : first;
    activateTab(initial, {fromHash:true});
  });

  // ---- hash 직접 전환 지원 ----
  window.addEventListener('hashchange', ()=>{
    const hash = (location.hash || '').slice(1);
    if(hash && document.getElementById(paneId(hash))){
      activateTab(hash, {fromHash:true});
    }
  });

  // ---- 방어 로그(선택) ----
  ['.tab-button','.tab-content','.slide-menu-overlay','.logo-button','.menu-button','.search-button','.search-overlay'].forEach(sel=>{
    const ok = $$(sel).length;
    if(!ok) console.debug('[UI]', sel, '없음/0개');
  });
})();
